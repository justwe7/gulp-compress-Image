...
function mapOverStylesAndTransformAllBackgroundImageDeclarations(styles, cb) {
	...
	function transformDeclaration(declaration, declarationIndex, declarations) {
		...
		if(transformedDeclaration.property === 'background-image' && /\?__s/i.test(transformedDeclaration.value)) {

            transformedDeclaration.value = transformedDeclaration.value.replace('?__s','');
            return cb(transformedDeclaration, declarationIndex, declarations);
        }
        // Background is a shorthand property so make sure `url()` is in there 且判断url是否有?__s后缀
        else if(transformedDeclaration.property === 'background' && /\?__s/i.test(transformedDeclaration.value)) {
